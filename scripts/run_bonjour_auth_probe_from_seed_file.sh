...
BATCH_TAG="${BATCH_TAG:-bonjour-auth-probe-followup}"
SEED_FILE="${SEED_FILE:-}"
...
AUTH_MAX_NODES="${AUTH_MAX_NODES:-100}"
AUTH_CONCURRENCY="${AUTH_CONCURRENCY:-8}"
AUTH_CHECKPOINT_EVERY="${AUTH_CHECKPOINT_EVERY:-25}"
...
if [[ -z "$TOKEN" ]]; then
  log "missing BONJOUR_TOKEN. Set BONJOUR_TOKEN or BONJOUR_TOKEN_SOURCE before running follow-up auth probe."
  exit 1
fi

SEED_COUNT="$("$NODE_BIN" -e 'const fs=require("fs"); console.log(JSON.parse(fs.readFileSync(process.argv[1], "utf8")).length)' "$SEED_FILE")"
log "follow-up batch started"
...
if [[ "$RUN_DELTA_PIPELINE" -eq 1 && "$DELTA_COUNT" -gt 0 ]]; then
  log "running delta pipeline from follow-up probe handles"
  RUN_DATE="$RUN_DATE" \
