...
BATCH_TAG="${BATCH_TAG:-bonjour-auth-frontier}"
DELTA_BATCH_TAG="${DELTA_BATCH_TAG:-${BATCH_TAG}-delta}"
SEED_FILE="${SEED_FILE:-}"
...
if [[ -z "$SEED_FILE" ]]; then
  log "missing SEED_FILE"
  exit 1
fi

if [[ ! -f "$SEED_FILE" ]]; then
  log "seed file not found: $SEED_FILE"
  exit 1
fi
...
if [[ -z "$TOKEN" ]]; then
  log "missing BONJOUR_TOKEN. Set BONJOUR_TOKEN or BONJOUR_TOKEN_SOURCE before running auth frontier."
  exit 1
fi

SEED_COUNT="$("$NODE_BIN" -e 'const fs=require("fs"); console.log(JSON.parse(fs.readFileSync(process.argv[1], "utf8")).length)' "$SEED_FILE")"
log "frontier batch started"
log "seed_file=$SEED_FILE seed_count=$SEED_COUNT auth_dir=$AUTH_DIR"
...
if [[ -f "$AUTH_DIR/manifest.json" ]]; then
  log "resuming auth frontier depth=$AUTH_DEPTH max_nodes=$AUTH_MAX_NODES concurrency=$AUTH_CONCURRENCY"
  BONJOUR_TOKEN="$TOKEN" "$NODE_BIN" --import tsx/esm apps/worker/src/cli.ts dump-bonjour-auth-handles \
    ...
    --max-node-retries "$AUTH_MAX_NODE_RETRIES" >> "$LOG_PATH" 2>&1
else
  log "starting auth frontier depth=$AUTH_DEPTH max_nodes=$AUTH_MAX_NODES concurrency=$AUTH_CONCURRENCY"
  BONJOUR_TOKEN="$TOKEN" "$NODE_BIN" --import tsx/esm apps/worker/src/cli.ts dump-bonjour-auth-handles \
    --seed-file "$SEED_FILE" \
    --depth "$AUTH_DEPTH" \
    --max-nodes "$AUTH_MAX_NODES" \
    --checkpoint-every "$AUTH_CHECKPOINT_EVERY" \
    --concurrency "$AUTH_CONCURRENCY" \
    --max-node-retries "$AUTH_MAX_NODE_RETRIES" \
    --output "$AUTH_DIR" >> "$LOG_PATH" 2>&1
fi

log "auth frontier finished"

if [[ ! -f "$AUTH_DIR/import-handles.json" ]]; then
  log "missing auth import-handles.json after auth frontier"
  exit 1
fi

log "filtering frontier handles against previous auth frontier + db coverage"
...
if [[ "$RUN_DELTA_PIPELINE" -eq 1 && "$DELTA_COUNT" -gt 0 ]]; then
  log "running delta pipeline from auth frontier handles delta_batch_tag=$DELTA_BATCH_TAG"
  RUN_DATE="$RUN_DATE" \
  BATCH_TAG="$DELTA_BATCH_TAG" \
  DELTA_IMPORT_HANDLES_PATH="$DELTA_IMPORT_HANDLES_PATH" \
  /bin/bash "$REPO_DIR/scripts/run_bonjour_delta_batch.sh" >> "$LOG_PATH" 2>&1
fi

log "frontier batch finished"
